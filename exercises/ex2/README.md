...
After completing these steps you will have added building blocks filterbar and table to your custom page.

(1) In Business Application Studio, on the Application Info tab Click **Open Guided Development**.

![](./images/image1.png)

(2) Enter search term 'filterbar' in tghe 'Search Guides' field.
...
Open drop down **CDS File** and select the file the annotations are to be inserted as shown.
...
(14) Click [](./images/image21.png) to get to step 2 in the development guide.
...
(22) In the opened file **Main.view.xml**, you can see the inserted building block xml snippet.\
...
(26) A separate browser tab is opened, showing the filter bar in top. Open a value help to make sure that filter values are shown.
...
Try to create the annotation as shown below, or alternatively just copy over the annotation snippet, anddon't forget the comma to separate from the previous annotation:
...
the code snippets can be manipulated which will result in an instant reload of the sample.

(30) In the code snippet it is shown that building blocks filter bar and table are semantically connected via the filter bar ID. In the code editor, select and copy the **macros:table** definition.
...
(37) On the **Add Page** dialog, open the drop down of field **Navigation**.

(38) Select **Travel (Travel) ![](./images/image63.png). This defines the context binding of the object page and\
